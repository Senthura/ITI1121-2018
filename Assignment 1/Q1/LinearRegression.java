...
public class LinearRegression {
	...
	public String currentHypothesis(){
		...
		return Double.toString(theta0) + (theta1 < 0.0 ? "" : "+") 
			+ Double.toString(theta1) + "x";
	}
	...
	public double currentCost(){
		...
		for(int i =0; i < nbreOfSamples; i++) {
			double val = hypothesis(samples[i]) - samplesValues[i];
			response += val * val;
		}
	...
	public void gradientDescent(double alpha, int numberOfSteps) {
		...
		for(int step = 0; step < numberOfSteps; step++){
			...
			for (int j = 0; j < nbreOfSamples; j++) {
				...
				tmp = hypothesis(samples[j])- samplesValues[j];
				sum1 += tmp;
				sum2 += tmp*samples[j];
			}	
			theta0 -= 2*alpha*sum1/nbreOfSamples;
			theta1 -= 2*alpha*sum2/nbreOfSamples; 
			iteration++;
		}
