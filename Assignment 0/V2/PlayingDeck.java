...
public class PlayingDeck {
	...
	public void setUnveiledPair(int p1, int p2, boolean unveiled){
		deck[p1-1].setUnveiled(unveiled);
		deck[p2-1].setUnveiled(unveiled);
		if(unveiled) {
			numberUncovered += 2;
		} else {
			numberUncovered -= 2;
	...
	public void cleanUpDeck(){
	    ...
	    for(int i = 0; i < deck.length - 1; i++) {
	    	first = deck[i];
	    	second = deck[i+1];
	    	if((first.getFace() == second.getFace()) && (first.getFace() != Card.MASK)) {
